...
from syne_tune.optimizer.baselines import AsynchronousSuccessiveHalving, SingleObjectiveScheduler
...
def make_domain(spec):
    ...
    # both bounds are inclusive in Syne Tune, unlike Ray Tune
    if spec_type == "randint":
        return randint(spec["lower"], spec["upper"])
    if spec_type == "lograndint":
        return lograndint(spec["lower"], spec["upper"])
...
def build_scheduler(parms, config_space):
    ...
    scheduler_name = scheduler_cfg.pop("name", "asha")

    common_kwargs = {
        "config_space": config_space,
        "metric": syne_tune_cfg["metric"],
        "do_minimize": syne_tune_cfg["do_minimize"],
    }

    if scheduler_name == "single_fidelity":
        return SingleObjectiveScheduler(**common_kwargs, **scheduler_cfg)

    if scheduler_name == "asha":
        # the resource is the epoch, and the most any trial gets is parms["epochs"]
        grace_period = scheduler_cfg.get("grace_period", 1)
        if grace_period > parms["epochs"]:
            raise ValueError(f"grace_period {grace_period} exceeds epochs {parms['epochs']}. "
                             "The first rung would never be reached - raise epochs or lower grace_period.")
        report_every = syne_tune_cfg["report_every"]
        if grace_period % report_every != 0:
            raise ValueError(f"grace_period {grace_period} is not a multiple of report_every "
                             f"{report_every}, so the first rung would never be observed.")

        common_kwargs["time_attr"] = syne_tune_cfg["time_attr"]
        common_kwargs["max_t"]     = parms["epochs"]
        return AsynchronousSuccessiveHalving(**common_kwargs, **scheduler_cfg)
